...
export default function DistrictsPage() {
  ...
  const load = useCallback(() => {
    ...
    fetch(`/api/master/districts?${params}`)
      .then((r) => r.json())
      .then((d: ListResponse) => {
        setRows(d.data || []);
  ...
  }, [page, filterProvince, filterCity, filterQ]);
  ...
  return (
    <div className="p-6 space-y-5 max-w-[1280px] mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Kecamatan</h2>
          <p className="text-slate-400 text-[13px]">Filter cascade: provinsi → kab/kota → pencarian</p>
        </div>
        <Link href="/master/districts/add">
          <Button>
            <Plus size={15} /> Tambah Kecamatan
          </Button>
        </Link>
      </div>
          ...
          <div className="lg:col-span-2">
            <Field label="Cari nama">
              <Input
                ...
                placeholder="Kecamatan, kab/kota, atau provinsi..."
              />
            </Field>
          </div>
        </div>
        ...
        <table className="w-full text-[13px]">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50/80 text-left text-[11px] uppercase tracking-wide text-slate-500">
              ...
              <th className="px-3 py-3 min-w-[120px]">Provinsi</th>
              <th className="px-3 py-3 min-w-[120px]">Kab/Kota</th>
                ...
                <tr key={r.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                  ...
                  <td className="px-3 py-2 text-right">
                    <div className="flex justify-end gap-2">
                      <Link href={`/master/districts/${r.id}`}>
