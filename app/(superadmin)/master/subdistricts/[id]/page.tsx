...
export default function EditSubdistrictPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/master/subdistricts/${id}`)
      .then((r) => r.json())
      .then((row) => {
        if (row && row.district_id != null) {
          setForm({
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    const r = await fetch(`/api/master/subdistricts/${id}`, {
      method: 'PUT',
  ...
  return (
    <div className="p-6 max-w-[800px] mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800">Edit Kelurahan / Desa</h2>
        ...
        <div className="p-6 space-y-5">
          ...
          <Field label="Kabupaten / Kota" required>
            <Select
              value={form.city_id}
              onChange={(e) =>
                setForm((f) => ({ ...f, city_id: e.target.value, district_id: '' }))
              }
