...
export default function ProvincesPage() {
  ...
  const load = useCallback(() => {
    ...
    fetch(`/api/master/provinces?${params}`)
      .then((r) => r.json())
      .then((d: ListResponse) => {
        setRows(d.data || []);
  ...
  const handleDelete = (id: number) => {
    confirmToast('Hapus provinsi ini?', {
      confirmLabel: 'Hapus',
      onConfirm: async () => {
        setDeleting(id);
        const r = await fetch(`/api/master/provinces/${id}`, { method: 'DELETE' });
        const j = (await r.json().catch(() => ({}))) as { error?: string };
        setDeleting(null);
        if (!r.ok) {
          toast.error(j.error || 'Gagal menghapus provinsi');
          return;
        }
  ...
  return (
    <div className="p-6 space-y-5 max-w-[1100px] mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Provinsi</h2>
          <p className="text-slate-400 text-[13px]">Kelola data provinsi (wilayah tingkat 1)</p>
        </div>
        <Link href="/master/provinces/add">
          <Button>
        ...
        <table className="w-full text-[13px]">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50/80 text-left text-[11px] uppercase tracking-wide text-slate-500">
              ...
              <th className="px-3 py-3">Nama Provinsi</th>
                ...
                <tr key={r.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                  ...
                  <td className="px-3 py-2 text-right">
                    <div className="flex justify-end gap-2">
                      <Link href={`/master/provinces/${r.id}`}>
