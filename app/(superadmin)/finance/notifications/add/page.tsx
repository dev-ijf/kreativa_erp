  ...
  return (
    <div className="p-6 max-w-[1200px] mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800">Create Notification Template</h2>
      ...
      <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-6">
        <div className="bg-white rounded-2xl border border-[#E2E8F1] shadow-sm overflow-hidden">
          <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Field label="Channel" required>
                <Select value={form.type} onChange={(e) => setForm((f) => ({ ...f, type: e.target.value as Channel }))}>
                  <option value="whatsapp">WhatsApp</option>
                  <option value="email">Email</option>
                </Select>
              </Field>
              <Field label="Trigger On" required>
                <Select value={form.trigger_event} onChange={(e) => setForm((f) => ({ ...f, trigger_event: e.target.value as TriggerKey }))}>
          ...
          <div className="bg-slate-50 border-t border-[#E2E8F1] p-5 flex justify-end gap-3">
            ...
            <Button loading={saving} type="submit" disabled={!form.name || !form.type || !form.trigger_event || !form.content || (form.type === 'email' && !form.subject)}>
              Update Template
            </Button>
          </div>
        </div>
