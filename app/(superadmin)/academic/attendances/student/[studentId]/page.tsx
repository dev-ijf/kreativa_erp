...
export default function AttendanceStudentDetailPage({ params }: { params: Promise<{ studentId: string }> }) {
  ...
  const load = () => {
    setLoading(true);
    fetch(`/api/academic/attendances?student_id=${studentId}`)
      .then((r) => r.json())
      .then((d) => {
  ...
  return (
    <div className="p-6 space-y-5 max-w-[1200px] mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800">Kehadiran siswa</h2>
      ...
      <DataTable
        ...
        loading={loading}
        rowKey={(r) => r.id}
        emptyText="Belum ada data kehadiran untuk siswa ini"
        searchable={false}
        showRowNumber
      />
    </div>
