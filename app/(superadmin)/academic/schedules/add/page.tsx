...
export default function AddSchedulePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (active) setForm((f) => ({ ...f, academic_year_id: String(active.id) }));
  ...
  useEffect(() => {
    if (!form.school_id) { setClasses([]); setForm((f) => ({ ...f, class_id: '' })); return; }
  ...
  return (
    <div className="p-6 max-w-[800px] mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800">Tambah Jadwal</h2>
        </div>
      </div>
