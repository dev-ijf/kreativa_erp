...
export default function AcademicSemestersPage() {
  ...
  const handleDelete = async (sid: number) => {
    confirmToast('Hapus semester ini?', {
      confirmLabel: 'Hapus',
      onConfirm: async () => {
        setDeleting(sid);
        const res = await fetch(`/api/academic/semesters/${sid}`, { method: 'DELETE' });
        setDeleting(null);
        if (!res.ok) {
  ...
  const columns = [
    ...
    { key: 'semester_label', label: 'Semester', sortable: true },
    {
      key: 'is_active',
      label: 'Aktif',
      render: (r: Row) =>
        r.is_active ? <span className="text-emerald-600 text-[12px] font-medium">Ya</span> : '–',
    },
    {
      key: 'actions',
      label: 'Aksi',
      ...
      render: (r: Row) => (
        <div className="flex justify-end gap-2">
          <Link href={`/academic/semesters/${r.id}`}>
    ...
    <div className="p-6 space-y-5 max-w-[1200px] mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-[13px]">Periode akademik</p>
        </div>
        <Link href="/academic/semesters/add">
          <Button>
            <Plus size={15} /> Tambah Semester
          </Button>
        </Link>
      </div>
