...
export default function AddClinicVisitPage() {
  ...
  useEffect(() => {
    fetch('/api/students?limit=100')
      .then((r) => r.json())
      .then((j) => setStudents((j?.data as typeof students) || []));
  }, []);

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    const res = await fetch('/api/academic/clinic-visits', {
      method: 'POST',
  ...
  return (
    <div className="p-6 max-w-[800px] mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800">Tambah Kunjungan UKS</h2>
        </div>
      </div>
